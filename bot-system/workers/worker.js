...
const worker = new Worker('tasks', async job => {
  ...
  try {
    ...
    if (job.name === 'visit') {
      ...
      if (!proxy) {
        throw new Error(
          'No proxy configured. Set PROXY_HOST/PROXY_PORT (and PROXY_USERNAME/PROXY_PASSWORD if auth is required) or pass proxy in job data.'
        );
      }
      ...
      try {
        ...
        //  применяем fingerprint ДО перехода (cookies — на домен целевого сайта)
